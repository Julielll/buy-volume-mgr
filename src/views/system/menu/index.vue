  ...
  <div class="app-container">
    <section class="card">
      ...
      <tree-table v-loading="loading" :data="data" :expand-all="expand" :columns="columns" size="small">
        ...
        <el-table-column v-if="checkPermission(['ADMIN','MENU_ALL','MENU_EDIT','MENU_DELETE'])" label="操作" width="160px" align="right">
          <template slot-scope="scope">
            <span>
              <el-button v-permission="['ADMIN','MENU_ALL','MENU_EDIT']" v-if="scope.row._level == 1" size="mini" type="success" icon="el-icon-plus" title="添加下级" @click="addChild(scope.row)"/>
            </span>
...
<script>
  ...
  methods: {
    ...
    addChild(row) {
      ...
      _this.getMenus()
      _this.form = { id: 0, component: '', name: '', sort: 999, pid: row.id, path: '', iframe: 'false', roles: [], icon: 'right-arrow' }
      _this.dialog = true
    },
    edit(data) {
      this.isAdd = false
