  ...
  <div class="app-container">
    <section class="card">
      ...
      <tree-table v-loading="loading" :data="data" :expand-all="expand" :columns="columns" size="small">
        ...
        <el-table-column v-if="checkPermission(['ADMIN','PERMISSION_ALL','PERMISSION_EDIT','PERMISSION_DELETE'])" label="操作" width="160px" align="right">
          <template slot-scope="scope">
            <span>
              <el-button v-permission="['ADMIN','PERMISSION_ALL','PERMISSION_EDIT']" v-if="scope.row._level == 1" size="mini" type="success" icon="el-icon-plus" title="添加下级" @click="addChild(scope.row)"/>
            </span>
...
<script>
  ...
  methods: {
    ...
    addChild(row) {
      ...
      _this.form = { id: 0, name: '', alias: '', pid: row.id }
      _this.dialog = true
    },
    edit(data) {
      this.isAdd = false
